...
{
	ReadyQueue.push(process1);
	ReadyQueue.push(process2);
	ReadyQueue.push(process3);
	ReadyQueue.push(process4);
	ReadyQueue.push(process5);
	ReadyQueue.push(process6);
	ReadyQueue.push(process7);
	ReadyQueue.push(process8);
}
...
void q_inc_by_one(queue<Process> &inputQueue)
{
...
//------------------------------START FUNCTIONS--------------------------------
//-----------------------------------------------------------------------------
//Starts the FCFS Process
void startFCFS()
{
	if (ReadyQueue.size() != 0)                 //If there are things in the ReadyQueue, Proceed
	{
		Process currentProcess = ReadyQueue.front();	// Gets A Process From The Front, Put's it in CurrentProcess
		ReadyQueue.pop();	                            // Deletes From Front of Ready Queue. 
...
// ------------- While CPU is running, you must account for IO Queue waiting. -------------
 ...
 // If the IO Burst Completes while CPU is running, it will be added back to the ready queue.
    ...
    //  Put it back in ready queue when done.
		ReadyQueue.push(IOQueue[IOQueueNumber]);
...
double getCPUUtilization()
{
	...
	cpuUtil = totalCPUUsage / getTotalTime;
	return cpuUtil;
}
